	...
	<div class="container-xxl min-vh-100 px-lg-4 pb-3">
		<div class="dotori-title d-flex align-items-center">
			<div class="d-flex flex-column justify-content-center">
				<h2 class="fs-5 fw-bold text-primary mb-0"><i class="feather icon-settings me-2"></i>사용자 수정</h2>
		...
		<form method="post" action="<?=BASE_URL?>/<?=DOTORI_CONTROLLER?>/ajax_edit_user" id="userEditForm" class="needs-validation" enctype="multipart/form-data" novalidate>
			...
			<div class="row gx-4">
					...
					<div class="card bg-soft-secondary shadow-sm mb-3">
						<div class="card-body">
							...
							<div class="mb-3">
								...
								<textarea name="user_memo" rows="3" class="form-control" id="user_memo" placeholder="메모"><?=$user['user_memo']?></textarea>
							</div>
							<div class="mb-3">
							...
							<div class="mb-3">
								...
								<input name="user_entertime" type="date" class="form-control" id="user_entertime" value="<?=($user['user_entertime'] ? date('Y-m-d', $user['user_entertime']) : '')?>">
							</div>
							<div class="mb-3">
								<label for="user_leavetime" class="form-label">퇴사일</label>
								<input name="user_leavetime" type="date" class="form-control" id="user_leavetime" value="<?=($user['user_leavetime'] ? date('Y-m-d', $user['user_leavetime']) : '')?>">
							</div>
							<div class="mb-3">
								...
								<select name="user_level" class="form-select" id="user_level" required>
									...
									<option value="1"<?if($user['user_level'] == "01"){?>selected<?}?>>일반사용자</option>
				...
				<div class="col-lg-9">
					<div class="card">
							...
							<div class="row g-2">
								...
								<div class="col-lg mb-3">
									...
									<div class="input-group">
										...
										<input name="password_check" type="password" class="form-control" id="password_check">
									</div>
									<div class="form-text">변경 시에만 입력하세요.</div>
								</div>
							</div>
							...
							<div class="row g-2">
								...
								<div class="col-lg mb-3">
									...
									<select name="user_sex" class="form-select" id="user_sex">
										...
										<option value="기타"<?if($user['user_sex'] == '기타'){?> selected<?}?>>기타</option>
									</select>
								</div>
							</div>
							<div class="row g-2">
						...
						<div class="card-body border-bottom">
							<input type="hidden" name="user_x" id="user_x" value="<?=$user['user_x']?>">
							<input type="hidden" name="user_y" id="user_y" value="<?=$user['user_y']?>">
							<div class="row g-2">
						...
						<div class="card-footer border-top-0">
							<div class="d-grid">
								<button type="submit" class="btn btn-warning btn-lg fw-bold" id="userEditConfirm">사용자 정보를 수정합니다.</button>	
							</div>
						</div>
					</div>
				</div>
	...
	document.getElementById('userEditForm').addEventListener('submit', (e) => {
		...
		.then((response) => {
			if (response.result == true){
				...
				Swal.fire({icon: 'error', title: '사용자 정보 수정 실패', text: response.message, timer: 1500})
			}
			return
		});
	});
