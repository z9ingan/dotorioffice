<div class="container-xxl py-3" style="height:calc(100vh - 3.5rem);">
	...
	<div class="alert alert-info">
		<i class="feather icon-alert-triangle"></i> 발주내용이 맞는지 꼭 검토하고 진행해주세요.
	</div>
...
?>
				<tr<?if(!$ex['product']){?> class="table-danger"<?}?>>
					...
					<td>
						<input type="hidden" name="excel_time[<?=$i?>]" value="<?=$po_time?>">
						<?=$po_time?>
					</td>
...
<div id="footer-control" class="fixed-bottom bg-light border-top" style="display:none;">
	<div class="container-xxl p-3">
		<div class="row">
			<div class="col">
				<button type="submit" class="btn btn-primary btn-block" id="controlExcelPoButton" form="excelControlForm" formaction="<?=BASE_URL?>/po/ajax_excel_po"><i class="feather icon-arrow-right-circle"></i> 선택한 것을 발주</button>
			</div>
		</div>
	</div>
</div>
	...
	<div class="modal-dialog modal-xl modal-dialog-centered">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="excelModalLabel">발주항목 엑셀 추가</h5>
			...
			<div class="modal-body">
				<form method="post" action="<?=BASE_URL?>/po/excel" id="excelForm" class="needs-validation" novalidate>
					<div class="mb-3">
						<label for="excel_time" class="form-label">발주일자<span class="text-danger">*</span></label>
...
$(function(){
	...
	// 엑셀 발주항목 추가
	$('#controlExcelPoButton').click(function(event){
