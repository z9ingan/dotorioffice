<div class="container-xxl py-3" style="height:calc(100vh - 3.5rem);">
	<div class="d-flex align-items-center justify-content-between mb-3">
		<h2 class="h4 font-weight-bold mb-0"><i class="feather icon-message-circle"></i> 코드 엑셀 추가</h2>
		<div>
			<button class="history-back btn btn-outline-dark" title="뒤로가기"><i class="feather icon-arrow-left"></i></button>
		</div>
	</div>
	<form method="post" action="<?=BASE_URL?>/setting/ajax_excel_product" id="excelForm" class="needs-validation" novalidate>
		<div class="mb-3">
		...
		request.done(function(data){
			...
			if(data.result == 'true'){
				Swal.fire({icon: 'success', title: '성공', text: '추가되었습니다.', timer: 1500})
					.then(function(){
						location.href = '<?=BASE_URL?>/setting/product';
					});
			}else{
